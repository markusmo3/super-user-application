...
public class WinSudo implements ISudo {
    ...
    public static String getMSDOSName(String fileName)
        ...
        Process process = Runtime.getRuntime()
                .exec("cmd /c for %I in (\"" + path + "\") do @echo %~fsI");
        process.waitFor();
    ...
    public static int executeAsAdministrator(String command, String args) {
        if (SU.debug) {
            System.err.println(String.format("executeAsAdministrator(%s,%s)",
                    String.valueOf(command), String.valueOf(args)));
        }
        ...
        if ("Windows Vista".equals(os) || "Windows 7".equals(os)) {
            try {
                ...
                if (SU.debug) {
                    System.err.println(
                            String.format("using lpDirectory = '%s'", lpDirectory));
                }
        ...
        if (!Kernel32X.INSTANCE.QueryInformationJobObject(hJob, 9, jeli.getPointer(),
                jeli.size(), lpReturnLength)) {
            lastError = Kernel32X.INSTANCE.GetLastError();
            errorMessage = Kernel32Util.formatMessageFromLastErrorCode(lastError);
            System.err.println("WARNING: Error querying job object: " + lastError + ": "
                    + errorMessage);
        }
        ...
        if (!Kernel32X.INSTANCE.SetInformationJobObject(hJob, 9, jeli.getPointer(),
                jeli.size())) {

            lastError = Kernel32X.INSTANCE.GetLastError();
            errorMessage = Kernel32Util.formatMessageFromLastErrorCode(lastError);
            System.err.println("WARNING: Error in SetInformationJobObject: " + lastError
                    + ": " + errorMessage);
        }
        ...
        if (!Kernel32X.INSTANCE.AssignProcessToJobObject(hJob, execInfo.hProcess)) {
            lastError = Kernel32X.INSTANCE.GetLastError();
            errorMessage = Kernel32Util.formatMessageFromLastErrorCode(lastError);
            System.err.println("WARNING: Error assigning process to job: " + lastError
                    + ": " + errorMessage);
        }
    ...
    public int sudo(String[] args, String[] additionalArgs) {
        try {
            ...
            String jarPath = WinSudo.class.getProtectionDomain().getCodeSource()
                    .getLocation().getPath();
            ...
            if (SU.debug) {
                ...
                System.err.println(
                        String.format("user.dir = '%s'", System.getProperty("user.dir")));
            }
            ...
            if (jcmd == null || jcmd.length() < 1) {
                ...
                if (decodedPath.endsWith(".jar")) {
                    ...
                    throw new IllegalStateException(
                            "Unable to perform elevation outside of jar");
                }
            } else {
            ...
            if (SU.debug) {
                System.err.println("pargs: " + pargs);
            }
            ...
            if ("Windows Vista".equals(os) || "Windows 7".equals(os)) {
                ...
                try {
                    ...
                    strparams = String.format("/C \"cd /D %s && %s %s\"",
