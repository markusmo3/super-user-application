...
public class PosixSudo implements ISudo {
    ...
    private static void javaToolOptionsSuck(List<String> args) {
        ...
        if (java_tool_options_suck != null && java_tool_options_suck.length() > 0) {
            List<String> java_tool_options_suck_list = stringToArgs(
                    java_tool_options_suck);
    ...
    public static void applySudoArgs(List<String> args) {
        ...
        if (have_display) {
            ...
            if (s.canExecute()) {
                ...
                args.add("--comment");
                args.add(
                        "Java application needs administrative privileges. Please enter your password");
                return;
            }
        }
    ...
    public int sudo(String[] args, String[] additionalArgs) {
        try {
            String jarPath = PosixSudo.class.getProtectionDomain().getCodeSource()
                    .getLocation().getPath();
            ...
            return executeAsAdministrator(System.getProperty("java.home") + "/bin/java",
                    sargs);
