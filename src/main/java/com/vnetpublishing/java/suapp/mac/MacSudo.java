...
public class MacSudo implements ISudo {
    ...
    public static int executeAsAdministrator(String command, String[] args) {
        ...
        pargs.add("-e");
        pargs.add(String.format("do shell script \"%s\" with administrator privileges",
                toParams(cargs.toArray(new String[cargs.size()]))));
    ...
    public int sudo(String[] args, String[] additionalArgs) {
        try {
            String jarPath = MacSudo.class.getProtectionDomain().getCodeSource()
                    .getLocation().getPath();
